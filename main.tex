\documentclass{moderncv}
\moderncvstyle{classic}

\usepackage[scale=0.75]{geometry}

\name{Joe}{Down}
\address{Flat 2, 406C, Hackney Road}{E2 7AP, London}{}
\title{Curriculum Vitae}
\email{joe-down@outlook.com}
\phone[mobile]{+44~7427~251~937}
\social[github]{github.com/joe-down}
\social[linkedin]{linkedin.com/in/joe-down}

\begin{document}
    \makecvtitle

    \section{Education}
        \cventry{2020--2024}{MEng Computer Science}{University College London}{}{grade}{desc}
        \cventry{2012--2019}{Secondary School}{The Royal Grammar School}{High Wycombe}{}{}

    \section{Experience}
        \cventry{Jun~2023--Aug~2023}{Associate Data Scientist}{Sky}{London}{}{desc}
        \cventry{Jun~2022--Sep~2022}{Junior Machine Learning Engineer}{Sky}{London}{}{desc}
        \cventry{2019--2020}{Tutor}{Explore Learning}{High Wycombe}{}{desc}

    \section{Skills}
        \cvitem{Programming Languages}{Python, C, C++, Java, JavaScript, TypeScript, Bash, SQL, Haskell, WebGL, TeX}
        \cvitem{Systems}{Linux (Arch, Nix, Debian), Arduino, Raspberry Pi, Windows, Meta Quest}
        \cvitem{Frameworks}{PyTorch, TensorFlow, Unity, Node.js, PyQt, Flask, Rospy, Nashpy, PettingZoo, OpenCV}
        \cvitem{Tools}{AWS (ECS, EC2, S3, SageMaker), Jupyter, JetBrains IDEs}

    \section{Other}
        \cvitemwithcomment{Scrum}{Professional Scrum Master I}{Issued Jul 2022}

\end{document}
